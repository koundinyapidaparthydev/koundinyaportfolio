...
| `GEMINI_TAILOR_MODEL` | `gemini-2.5-flash` | Tailor + post-tailor scoring + verify (falls back to `GEMINI_MODEL` on 404) |
...
- `GEMINI_TAILOR_MODEL` — model for tailor + verify scoring (default `gemini-2.5-flash`, auto-fallback to `GEMINI_MODEL`)
